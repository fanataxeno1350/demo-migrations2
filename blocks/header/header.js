...
export default async function decorate(block) {
  ...
  // Press Releases (within Newsroom mega-menu)
  const newsroomLi = navUl.querySelector('a[href*="newsroom"]')?.closest('li');
  if (newsroomLi) {
    const newsroomMegaMenu = newsroomLi.querySelector('.mega-menu .left-div'); // Corrected class name
    if (newsroomMegaMenu) {
  ...
  // Find the contact link from itemRows
  const contactUsLinkItem = contactLinkItems.find(
    (row) => row.children[1].textContent.trim().toLowerCase() === 'contact us',
  );
  if (contactUsLinkItem) {
    const [linkCell, labelCell] = [...contactUsLinkItem.children];
    const foundLink = linkCell.querySelector('a');
    if (foundLink) mailLinkMobile.href = foundLink.href;
    mailLinkMobile.textContent = labelCell.textContent.trim();
    moveInstrumentation(contactUsLinkItem, mailLiMobile);
  } else {
    // Fallback if "Contact Us" is not found in itemRows
  ...
  if (contactUsLinkItem) {
    const [linkCell, labelCell] = [...contactUsLinkItem.children];
    const foundLink = linkCell.querySelector('a');
    if (foundLink) mailLinkDesktop.href = foundLink.href;
    // For desktop, the original HTML has an SVG, not text.
    // We'll keep the SVG and not set textContent for desktop.
    moveInstrumentation(contactUsLinkItem, mailLiDesktop);
  } else {
  ...
  searchForm.action = 'https://www.mahindra.com/search'; // This URL is hardcoded in original HTML
  ...
  popularKeywordsLabel.textContent = 'Popular Keywords:'; // Hardcoded in original HTML
  ...
  recommendedLabel.textContent = 'Recommended for you:'; // Hardcoded in original HTML
  ...
  const searchToggleElements = [searchLinkMobile, searchLinkDesktop];
  searchToggleElements.forEach((el) => {
    el.addEventListener('click', (e) => {
      e.preventDefault();
      const parentLi = el.closest('li.search');
      parentLi.classList.toggle('active');
      parentLi.querySelector('.search-screen-wrap').classList.toggle('active');
    });
  });
  ...
  // Optimization for all images in the block

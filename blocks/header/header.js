...
function transformNestedLists(rootUl) {
  rootUl.querySelectorAll('li').forEach((li) => {
    ...
    if (nested) {
      ...
      subWrap.classList.add('has-sub-child'); // Added 'active' class here
...
export default function decorate(block) {
  ...
  // Fixed fields: mainLogo, mainLogoLink, yearLogo, yearLogoLink
  const [mainLogoRow, mainLogoLinkRow, yearLogoRow, yearLogoLinkRow, ...itemRows] = children;
  ...
  if (mainLogoPicture) {
    ...
    if (mainLogoImg) {
      ...
      // The original HTML has 'hiddenlogo1' on the img, but createOptimizedPicture replaces the img.
      // We need to add it to the new img.
  ...
  if (yearLogoPicture) {
    const yearLogoImg = yearLogoPicture.querySelector('img');
    if (yearLogoImg) {
      const optimizedYearLogo = createOptimizedPicture(yearLogoImg.src, yearLogoImg.alt, false, [{ width: '74' }]);
      optimizedYearLogo.querySelector('img').classList.add('hiddenlogo1', 'years-80');
      moveInstrumentation(yearLogoImg, optimizedYearLogo.querySelector('img'));
      yearLogoLink.append(optimizedYearLogo);
    }
  }
  ...
  itemRows.forEach((row) => {
    ...
    // Order of detection matters: more specific checks first.
    // press-release-item has 4 cells, but also specific content.
    // navigation-item also has 4 cells.
    if (cells.length === 4 && cells[0].querySelector('p') && cells[1].querySelector('a')) { // press-release-item
      pressReleaseItems.push(row);
    } else if (cells.length === 4) { // navigation-item
      navigationItems.push(row);
    } else if (cells.length === 2) { // contact-link-item
  ...
  navigationItems.forEach((row) => {
    ...
    if (hierarchyUl) {
      // Apply classes to the hierarchyUl and its children before transforming
      hierarchyUl.classList.add('about-us-sub-nav'); // Example from original HTML
      hierarchyUl.querySelectorAll('li').forEach(hLi => {
        hLi.classList.add('top-level-li'); // Example from original HTML
        const hAnchor = hLi.querySelector('a');
        if (hAnchor) {
          // Add any specific classes for anchors if needed
        }
        const hNestedUl = hLi.querySelector('ul');
        if (hNestedUl) {
          const hSpan = document.createElement('span');
          hSpan.innerHTML = `<svg viewBox="-23.5 -23.5 122.80 122.80" fill="#000000" stroke="#000000" stroke-width="4.851456000000001"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round" stroke="#CCCCCC" stroke-width="0.30321600000000004"></g><g id="SVGRepo_iconCarrier"> <g id="Group_65" data-name="Group 65" transform="translate(-831.568 -384.448)"> <path id="Path_57" data-name="Path 57" d="M833.068,460.252a1.5,1.5,0,0,1-1.061-2.561l33.557-33.56a2.53,2.53,0,0,0,0-3.564l-33.557-33.558a1.5,1.5,0,0,1,2.122-2.121l33.556,33.558a5.53,5.53,0,0,1,0,7.807l-33.557,33.56A1.5,1.5,0,0,1,833.068,460.252Z" fill="#030408"></path> </g> </g></svg>`;
          hLi.append(hSpan);
        }
      });
  ...
  contactLinkItems.forEach((row) => {
    ...
    const link = document.createElement('a');
    link.href = linkCell.querySelector('a')?.href || '#';
    link.innerHTML = `<svg version="1.1" id="Layer_1" x="0px" y="0px" viewBox="0 0 48 38.4" style="enable-background:new 0 0 48 38.4;" xml:space="preserve" width="21" height="21" xmlns:xlink="http://www.w3.org/1999/xlink">
  ...
  // Add event listeners for nested sub-menus within mega menu
  // Corrected selector to target the span within the first-level-li that has a nested ul
  navUl.querySelectorAll('.has-sub-child > ul > li.top-level-li > span').forEach((span) => {
    span.addEventListener('click', (e) => {
      e.preventDefault();
      e.stopPropagation();
      const parentLi = span.closest('li.top-level-li');
      parentLi.classList.toggle('active');
      // The original HTML has 'has-inner-sub-child' on the div containing the nested ul
      parentLi.querySelector('.has-sub-child')?.classList.toggle('active-child');
    });
  });
}

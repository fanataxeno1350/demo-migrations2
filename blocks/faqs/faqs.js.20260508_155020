...
export default function decorate(block) {
  ...
  // section.classList.add('faqs'); // Removed: outer block already has this class
  ...
  children.slice(1).forEach((row, index) => {
    ...
    button.type = 'button';
    button.setAttribute('data-bs-toggle', 'collapse'); // Added Bootstrap data attribute
    button.setAttribute('data-bs-target', `#${collapseId}`); // Added Bootstrap data attribute
    button.setAttribute('aria-expanded', index === 0 ? 'true' : 'false');
    button.setAttribute('aria-controls', collapseId);

    // If not the first item, add 'collapsed' class to button
    if (index !== 0) {
      button.classList.add('collapsed');
    }

    // Removed manual toggle behavior, Bootstrap handles it via data attributes
  ...
  // This part seems unrelated to the accordion logic and might be generic image optimization.
  // Keeping it as is, assuming it's intended for any images within the block.

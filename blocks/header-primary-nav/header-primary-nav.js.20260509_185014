...
function transformNestedLists(rootUl) {
  rootUl.querySelectorAll('li').forEach((li) => {
    ...
    if (nested) {
      ...
      subWrap.classList.add('transition-display', 'max-lg:overflow-auto', 'max-lg:w-full', 'max-lg:h-[calc(100dvh-var(--navbar-height))]', 'duration-200', 'hidden', 'allow-discrete', 'opacity-0', 'starting:group-[.active]:opacity-0', 'group-[.active]:opacity-100', 'group-[.active]:block', 'absolute', 'bg-surface-navbar', 'inset-x-0', 'top-full', 'py-12', 'lg:py-2xl', 'shadow-md', 'border-t', 'border-t-stroke-muted'); // Using classes from original subnav element
      ...
      if (trigger) {
        ...
        button.setAttribute('aria-controls', `subnav-${Math.random().toString(36).substring(2, 9)}`); // Unique ID

        const triggerContent = trigger.textContent.trim();
        trigger.remove();
        button.textContent = triggerContent;
        ...
        button.addEventListener('click', (e) => {
          ...
          subWrap.classList.toggle('group-[.active]:block', isActive);
          subWrap.classList.toggle('group-[.active]:opacity-100', isActive);
          subWrap.classList.toggle('opacity-0', !isActive);
        });
        li.prepend(button);
      }
    } else if (anchor) {
      // For flat links, ensure they have the correct classes
      anchor.classList.add('link', 'text-foreground', 'text-p2', 'xl:text-p1', 'transition-display', 'hocus:underline', 'hocus:text-foreground', 'motion-safe:not-focus-visible:transition-underline');
    }
  });
}

export default function decorate(block) {
  // Reorder children destructuring to match the BlockJson model
  const [logoLinkRow, primaryNavPlaceholder, actionNavPlaceholder, socialLinksPlaceholder, ...itemRows] = [...block.children];
  ...
  // Read href from the anchor inside the logoLinkRow's first cell
  logoLink.href = logoLinkRow.children[0]?.querySelector('a')?.href || '/';
  moveInstrumentation(logoLinkRow, logoLink);

  logoLink.innerHTML = `
    <span class="sr-only">World Wildlife Fund</span>
    <svg aria-hidden="true" class="brand-logo-svg w-full h-auto max-w-[39px] max-h-[56px] sm:max-w-[41px] sm:max-h-[60px]" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 81.63 122.37">
  ...
  mainNavWrapper.classList.add('lg:col-start-2', 'lg:col-span-14', 'flex', 'justify-start', 'items-center');

  const navContentWrapper = document.createElement('div');
  navContentWrapper.classList.add('flex', 'justify-between', 'items-center', 'gap-4', 'xl:gap-grid-gutter', 'w-full');
  mainNavWrapper.append(navContentWrapper);
  ...
  moveInstrumentation(primaryNavPlaceholder, primaryNavUl); // Move instrumentation from placeholder

  // Filter for primary-nav-item (2 cells, second cell contains <ul>)
  const primaryNavItems = itemRows.filter(row => row.children.length === 2 && row.children[1].querySelector('ul'));
  primaryNavItems.forEach((row) => {
    ...
    // The transformNestedLists function expects a root <ul>, so we pass the actual ul from the cell
    const subList = hierarchyTreeCell.querySelector('ul');
    if (subList) {
      // Create a temporary div to hold the content for instrumentation and transformation
      const tempDiv = document.createElement('div');
      tempDiv.innerHTML = hierarchyTreeCell.innerHTML;
      moveInstrumentation(hierarchyTreeCell, tempDiv); // Move instrumentation from the original cell
      transformNestedLists(tempDiv.querySelector('ul')); // Pass the ul inside the tempDiv
      while (tempDiv.firstChild) {
        li.append(tempDiv.firstChild); // Append all transformed children
      }
    }
    primaryNavUl.append(li);
    moveInstrumentation(row, li); // Move instrumentation from the item row to the new li
  });
  navContentWrapper.append(desktopMenu);

  const rightNavWrapper = document.createElement('div');
  rightNavWrapper.classList.add('flex', 'items-center');
  navContentWrapper.append(rightNavWrapper);
  ...
  // Action Nav
  const actionNavDesktopMenu = document.createElement('div');
  actionNavDesktopMenu.classList.add('lg:ml-2', 'xl:ml-5');
  actionNavDesktopMenu.setAttribute('data-desktop-menu', '');
  actionNavDesktopMenu.setAttribute('data-keep-open-mobile', '');
  const actionNavUl = document.createElement('ul');
  actionNavUl.classList.add('flex', 'flex-row', 'gap-1.5', 'xl:gap-4', 'items-center');
  actionNavUl.setAttribute('data-primary-nav', '');
  actionNavDesktopMenu.append(actionNavUl);
  moveInstrumentation(actionNavPlaceholder, actionNavUl); // Move instrumentation from placeholder

  // Filter for action-nav-item (2 cells, second cell contains <a>)
  const actionNavItems = itemRows.filter(row => row.children.length === 2 && row.children[1].querySelector('a'));
  actionNavItems.forEach((row) => {
    const [labelCell, buttonLinkCell] = [...row.children];
    const li = document.createElement('li');
    li.classList.add('group', 'list-none', 'leading-[1.1]');
    li.setAttribute('data-has-subnav', ''); // This might be incorrect if action items don't have subnav

    const buttonLink = buttonLinkCell.querySelector('a');
    if (buttonLink) {
      const anchor = document.createElement('a');
      anchor.classList.add('button', 'text-14', 'xl:text-18', 'p-[15px]', 'xl:px-8', 'xl:py-4', 'forced-colors:border', 'button--sedona'); // Assuming a default button style
      anchor.href = buttonLink.href;
      anchor.textContent = labelCell.textContent.trim();
      li.append(anchor);
      moveInstrumentation(row, li); // Move instrumentation from the item row to the new li
    }
    actionNavUl.append(li);
  });
  rightNavWrapper.append(actionNavDesktopMenu);

  // Social Links (newly added based on model)
  const socialLinksItems = itemRows.filter(row => row.children.length === 2 && !row.children[1].querySelector('ul') && !row.children[1].querySelector('a')); // Assuming social links are the remaining 2-cell rows without ul/a
  if (socialLinksItems.length > 0) {
    const socialLinksWrapper = document.createElement('div');
    socialLinksWrapper.classList.add('flex', 'gap-sm'); // Example classes from ORIGINAL HTML
    moveInstrumentation(socialLinksPlaceholder, socialLinksWrapper); // Move instrumentation from placeholder

    socialLinksItems.forEach((row) => {
      const [platformCell, linkCell] = [...row.children];
      const socialLink = linkCell.querySelector('a');
      if (socialLink) {
        const anchor = document.createElement('a');
        anchor.href = socialLink.href;
        anchor.classList.add('transition-colors', 'hover:cursor-pointer', 'theme-focus-outline', 'outline-none', 'fill-foreground', 'hocus:fill-foreground-accent'); // Classes from ORIGINAL HTML
        ...
        // Map platform text to SVG icon names and SR text
        if (platform.includes('facebook')) {
          iconName = 'facebook';
          srOnlyText = 'Facebook profile';
        } else if (platform.includes('twitter') || platform.includes('x.com')) {
          iconName = 'twitter';
          srOnlyText = 'X.com profile';
        } else if (platform.includes('instagram')) {
          iconName = 'instagram';
          srOnlyText = 'Instagram profile';
        } else if (platform.includes('linkedin')) {
          iconName = 'linkedin';
          srOnlyText = 'Linkedin profile';
        } else {
          // Fallback for unknown platforms, or skip
        ...
        anchor.innerHTML = `
          ...
          <svg class="icon icon--${iconName} size-7" aria-hidden="true">
            <use xlink:href="#${iconName}" href="#${iconName}"></use>
          </svg>
        `;
        socialLinksWrapper.append(anchor);
        moveInstrumentation(row, anchor); // Move instrumentation from the item row to the new anchor
      }
    });
    rightNavWrapper.append(socialLinksWrapper);
  }
  ...
  // Event listeners for search and mobile menu
  ...
  mobileMenuToggle.addEventListener('click', () => {
    ...
    if (toggleTextSpan) {
      toggleTextSpan.textContent = isExpanded ? mobileMenuToggle.dataset.mobileMenuOpenText : mobileMenuToggle.dataset.mobileMenuCloseText;
    }
    // Add logic to show/hide mobile menu content here if needed
  });

  nav.append(mainNavWrapper);

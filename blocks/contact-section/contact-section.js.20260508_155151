...
export default async function decorate(block) {
  ...
  // Filter item rows based on cell count to distinguish sub-components
  const socialLinkRows = itemRows.filter((row) => row.children.length === 1);
  const externalEmbedRows = itemRows.filter((row) => row.children.length === 3);
  ...
  contactDetailsCol.append(locationWrapper);

  const locationText = document.createElement('p');
  moveInstrumentation(locationRow, locationText);
  locationText.style.paddingLeft = '19px';
  ...
  emailWrapper.append(emailSvg);

  const emailLink = document.createElement('a');
  const foundEmailLink = emailLinkRow.children[0]?.querySelector('a');
  if (foundEmailLink) {
    emailLink.href = foundEmailLink.href;
    emailLink.textContent = foundEmailLink.textContent.trim(); // Use textContent from the link itself
  }
  ...
  socialLinkRows.forEach((row) => {
    const [linkCell] = [...row.children]; // Destructure for fixed schema social-link-item
    const socialItem = document.createElement('li');
    socialItem.classList.add('social-items');
    ...
    socialAbout.append(socialItem);
  });

  externalEmbedRows.forEach(async (row) => {
    const [embedUrlCell, embedKindCell, embedConfigCell] = [...row.children]; // Destructure for fixed schema external-embed
    ...
    contactDetailsCol.append(embedWrapper);
  });
